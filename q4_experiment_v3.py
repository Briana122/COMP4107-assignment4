...
class Autoencoder(torch.nn.Module):
    def __init__(self, batchnorm_layers=0, dropout_rate=0.0, output_activation="sigmoid"):
        super().__init__()
        ...
        # 2 = encoder conv2
        # 3 = decoder deconv1
        # 4 = decoder deconv2 (usually skipped for BN)
        ...
        if batchnorm_layers >= 4:
            decoder_layers.append(torch.nn.BatchNorm2d(3))
...
# testing number of layers using batch normalization
# 0 = none
# 1 = first encoder layer only
# 2 = both encoder layers
# 3 = both encoder layers + first decoder layer
# 4 = all conv/deconv layers
# =========================
batchnorm_values = [0, 1, 2, 3, 4]
train_results = []
val_results = []

for num_bn_layers in batchnorm_values:
    model, train_loss, val_loss = train_autoencoder(
        train_dataset_path,
        preprocessing="zero_one",
        batchnorm_layers=num_bn_layers
    )
    train_results.append(train_loss)
    val_results.append(val_loss)
...
run_experiment(
    ...
    "Batch Normalization Layers vs Performance"
)
