...
# Dataset with configurable preprocessing for Question 4(a)
class Linnaeus5DatasetQ4(Dataset):
    def __init__(self, dataset_directory, preprocessing="zero_one"):
        ...
        # Question 4(a) preprocessings:
        # 1) zero_one: pixel values in [0, 1]
        # 2) mean_std: centered to mean 0, std 1 style scaling using mean=0.5, std=0.5
        if preprocessing == "zero_one":
...
class Autoencoder(torch.nn.Module):
    def __init__(self, use_batchnorm=False, dropout_rate=0.0, output_activation="sigmoid"):
        super().__init__()
        ...
        if use_batchnorm:
            encoder_layers.append(torch.nn.BatchNorm2d(16))
        ...
        if use_batchnorm:
            encoder_layers.append(torch.nn.BatchNorm2d(32))
        encoder_layers.append(torch.nn.ReLU())
        if dropout_rate > 0:
            encoder_layers.append(torch.nn.Dropout2d(dropout_rate))
        ...
        if use_batchnorm:
            decoder_layers.append(torch.nn.BatchNorm2d(16))
...
def get_output_activation(preprocessing):
    # Match output range to target range
    if preprocessing == "zero_one":
        return "sigmoid"   # outputs in [0, 1]
    elif preprocessing == "mean_std":
        return "tanh"      # outputs in [-1, 1]
    else:
        raise ValueError(f"Unknown preprocessing option: {preprocessing}")
...
# =========================
preprocessing_options = ["zero_one", "mean_std"]
train_results = []
val_results = []
...
# baseline preprocessing: zero_one
# =========================
batchnorm_options = [False, True]
batchnorm_labels = ["No", "Yes"]
train_results = []
val_results = []

for option in batchnorm_options:
    model, train_loss, val_loss = train_autoencoder(
        train_dataset_path,
        preprocessing="zero_one",
        use_batchnorm=option
    )
    train_results.append(train_loss)
    val_results.append(val_loss)
...
# =========================
# (c) L1/L2 regularization experiment
# baseline preprocessing: zero_one
# =========================
...
regularization_labels = ["none", "l1_1e-5", "l2_1e-5", "l2_1e-4"]
train_results = []
val_results = []

for reg_type, reg_lambda in regularization_configs:
    actual_reg = None if reg_type == "none" else reg_type
    model, train_loss, val_loss = train_autoencoder(
        train_dataset_path,
        preprocessing="zero_one",
        regularization=actual_reg,
